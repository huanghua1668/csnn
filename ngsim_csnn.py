...
def pre_train(model, optimizer, dl_train, dl_test, x_train, y_train, run, outputDir, maxEpoch=200):
    losses = []
    accuracies = []
    losses_validate = []
    accuracies_validate = []
    ...
    plot_save_loss(losses, losses_validate, outputDir+'/loss_csnn_pretrain_2_csnn_layers_run{}_affine_false.png'.format(run))
    plot_save_acc(accuracies, accuracies_validate, outputDir+'/acc_csnn_pretrain_2_csnn_layers_run{}_affine_false.png'.format(run))
...
if __name__ == "__main__":
    ...
    if not pretrained:
        for run in range(runs):
            ...
            model = Net(inputs, hiddenUnits)
            optimizer = torch.optim.Adam(model.parameters(), lr=learningRate,
                                         weight_decay=l2Penalty)
            pre_train(model, optimizer, dl_train, dl_test, x_train, y_train, run, outputDir, maxEpoch=10)
    ...
    if not trained:
        ...
        for run in range(runs):
            ...
            accs= []
            for epoch in epochs:
                PATH = outputDir + '/csnn_2_csnn_layers_run{}_epoch{}_r2{:.1f}_maxAlpha{:.1f}_affine_false.pth'.format(run, epoch, r2, maxAlpha)
                if epoch==0:
                    PATH = outputDir+'/csnn_2_csnn_layers_run{}_epoch{}_affine_false.pth'.format(run, epoch)
                l = torch.load(PATH)
                ...
                dir = outputDir + '/roc_csnn_2_csnn_layers_run{}_epoch{}_r2{:.1f}_maxAlpha{:.1f}_affine_false.png'.format(run, epoch, r2, maxAlpha)
        ...
        dir = outputDir + '/mean_std_accs_aucs_csnn_2_csnn_layers_r2{:.1f}_maxAlpha{:.1f}_affine_false.npz'.format(r2, maxAlpha)
